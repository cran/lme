	   ...
	   verbose= FALSE)
{
  ...
  ##
  nlFrame <- new.frame(list(model = gnlsModel,
			    data = dataMod,
			    plist = plist,
			    beta = spar,
  ...
  w <- eval(modelResid[[2]], local = nlFrame)
  ...
  ## alternating algorithm
    ...
    if (length(oldPars <- coef(gnlsSt)) > 0) {
      aMs <- ms(~-logLik(gnlsSt, gnlsPars),
                start = list(gnlsPars = c(coef(gnlsSt))),
      ...
      aConv <- coef(gnlsSt) <- aMs$parameters
      convIter <- aMs$numIter <- aMs$flags[31]
      if (verbose) {
        cat("\n**Iteration", numIter)
        cat("\n")
        cat("GLS step: Objective:", format(aMs$value),
    ...
    if ((max(aConv) <= controlvals$tolerance) ||
        (aConv["params"] <= controlvals$tolerance && aMs$numIter == 1)) {
      convResult <- 0
      break
    }
	   ...
	   verbose = FALSE)
{
  ...
  if (is.null(thisCall$correlation) &&
...
residuals.gnlsStruct <-
  function(object)
{
  c(eval(attr(object, "model")[[2]], local = attr(object, "local")))
}
