...
#	     eval(parse(text = paste("~1", deparse(groups[[2]]), sep = "|"))))
  ...
  repeat {
    ...
    aNlm <- nlm(f = function(lmePars) -logLik(lmeSt, lmePars),
                p = c(coef(lmeSt)),
                hessian = TRUE,
                ndigit = 6,             # using numerical gradient internally
  ...
  function(object, level = 0.95, which = c("all", "var-cov", "fixed"))
{
  ...
  if (which != "var-cov") {		# fixed effects included
    ...
    val <-
      list(fixed = structure(array(c(est - len, est, est + len),
  ...
  if (which != "fixed") {		# variance-covariance included
    ...
    natInt[["sigma"]] <-
      ...
      structure( exp(origInt[nP,]), label = "Within-group standard error:" )
	   ...
	   control)
{
  ...
  if (!is.null(thisCall$fixed)) {
    thisCall$fixed <- update(formula(object), thisCall$fixed)
  }
