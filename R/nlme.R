### $Id: nlme.q,v 1.28 1999/05/13 15:46:20 pinheiro Exp $
###
	   ...
	   verbose= FALSE)
{
  ...
  if (missing(groups)) {
    this.call[["groups"]] <- groups <- getGroupsFormula(model)
  }
  ...
  val <- do.call("nlme.formula", this.call)
	   ...
	   verbose= FALSE)
{
  ...
  rnames <- structure(vector("list", Q), names = namGrp)
  for(i in 1:Q) {
  ...
  ##
  contr <- list()
  plist <- structure(vector("list", length(pnames)), names = pnames)
  for (nm in pnames) {
    this <- list(fixed = !is.null(fixed[[nm]]),
                 random = lapply(ranForm, function(el, nm) !is.null(el[[nm]]),
                   nm = nm))
    if (this[["fixed"]]) {
    ...
    if (any(unlist(this[["random"]]))) {
      for(i in 1:Q) {
        ...
        if (length(wch) == 1) {           # only one formula for nm at level i
          ...
          this[["random"]][[i]] <-
            lapply(ranForm[[i]][wch], function(el, data) {
  ...
  ##
  rn <- wchRnames <- structure(vector("list", Q), names = namGrp)
  for(i in 1:Q) {
    rn[[i]] <- character(0)
  ...
  sran <- structure(vector("list", Q), names = namGrp)
  ...
  rmap <- rmapRel <- structure(vector("list", Q), names = namGrp)
  n1 <- 1
  startRan <- 0
  for(i in 1:Q) {
    wchRnames[[i]][] <- 0
  ...
  ## alternating algorithm
    ...
    aMs <- ms(~-logLik(nlmeSt, nlmePars),
              ...
              control = list(rel.tolerance = controlvals$msTol,
                maxiter = controlvals$msMaxIter, scale = controlvals$msScale),
    ...
    work <- .C("fit_nlme", 
	       thetaPNLS = c(as.vector(unlist(sran)), sfix),
...
#      break
    ...
    if ((max(aConv) <= controlvals$tolerance) ||
        (aConv["fixed"] <= controlvals$tolerance && aMs$numIter == 1)) {
      convResult <- 0
      break
    }
            ...
            plist[[nm]]$random[[i]] <- lapply(ranForm[[i]][wch],
                           function(el, data) {
                             if (as.name(el[[3]]) == "1") {
                               ...
                               model.matrix(asOneSidedFormula(el[[3]]),
  ...
  function(object, level = Q, conLin = attr(object, "conLin"), ...)
{
  ...
  for(i in 1:length(level)) {
    ...
    val[, i] <- c(eval(attr(object, "model")[[2]], local=loc))
  }
  val
}
